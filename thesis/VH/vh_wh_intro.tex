...
Backgrounds in this analysis are estimated using several different methods. The first background category includes prompt processes such as $ZZ$, $VVV$, $ttV$, and $tZ$, where all selected leptons originate from vector boson decays, and are estimated directly from MC simulation.

The second, and main, background category consists of $WZ/W\gamma^{*}$ production. This background is estimated with a hybrid method using MC simulation, corrected with normalization factors (NFs) derived using a data-driven approach with a dedicated control region (CR) targeting this background. The method is presented in Section~\ref{sec:vh_wh_wz_control_regions}.
