...
\begin{table}[htp]
  ...
  \begin{tabular}{l|l}
    ...
    \hline
  \end{tabular}
  \caption{Control region definitions used in calculating the fake electron scale factors. The anti-ID version is obtained by requiring the presence of an anti-ID electron as defined in Table~\ref{tab:wh_id_antiid_selection}.}\label{tab:wh_electron_id_antiid_selection}
\end{table}
...
\begin{figure}[htp]
  ...
  \caption{Distributions of the electron \pt{} in the ID and anti-ID CR's used for the fake electron scale factor calculation. The $\gamma$ conversion CRs are shown in (a) and (b), the light flavor CRs in (c) and (d), and the heavy flavor CRs in (e) and (f).}\label{fig:VH_electron_id_crs}
\end{figure}

The final CR used for obtaining the fake electron background estimate in the SR is identical to the SR except for requiring one anti-ID electron. This method assumes that for a given pair of ID and anti-ID CRs targeting fake background $i$, the following equation holds for each \pt{} bin:
...
\begin{equation}
  ...
  \label{eq:vh_wh_electron_likelihood}
\end{equation}
where
\begin{itemize}
  \item $N_{ij}^{\mathrm{ID, data}}$ is the data yield in the ID CR for process $j$ in \pt{} bin $i$,
  \item $N_{ij}^{\mathrm{ID, estimated}}(\vec{\theta}) = N_{ij}^{\mathrm{ID, prompt}}(\vec{\theta}) + \sum_{f}SF^{i}_{f} \cdot N_{ij}^{\mathrm{anti-ID, fake f}}$ is the sum of the prompt lepton background yields in the ID CR $j$ and the fake lepton background yield in the anti-ID CR $j$ multiplied by the scale factor for that \pt{} bin,
  \item $N_{ij}^{\mathrm{ID, prompt}}(\vec{\theta}) = \sum_{k}N_{ij}^{\mathrm{ID, prompt k}} \cdot \theta_{k}$, is the sum of the yields with normalization factor $\theta_{k}$ of each prompt process $k$ in the ID CR $j$ and \pt{} bin $i$, with $k = WZ, ZZ, VVV, t\bar{t}V, VH$, and $t\bar{t}H$.
\end{itemize}
The first term is used to extract the scale factor $SF^{i}$, by modeling the event yield in each \pt{} bin $i$ in the ID scale factor CR\@. The second term is a Gaussian term used to constrain the normalization of prompt lepton background $k$ via a set ($\vec{\theta}_{k}$) of NPs ($\theta_{k}$). The mean $\tilde{\theta}_{k}$ is set to 1.0 and the standard deviation $\Delta{\theta_{k}}$ is equal to $\frac{\Delta_{\sigma_{k}}}{\sigma_{k}}$ which is the relative uncertainty in the cross section of process $k$. The normalizations of the dominant prompt lepton processes are constrained using their relative cross-section uncertainties: $\frac{\Delta\sigma_{t\bar{t}V}}{\sigma_{t\bar{t}V}}$ = 13\%~\cite{ttVNLOTwiki}, $\frac{\Delta\sigma_{t\bar{t}H}}{\sigma_{t\bar{t}H}}$ = 10\%~\cite{CERNYR4Twiki}, $\frac{\Delta\sigma_{WZ}}{\sigma_{WZ}}$ = 5\%~\cite{STDM-2018-03}, $\frac{\Delta\sigma_{ZZ}}{\sigma_{ZZ}}$ = 6.5\%~\cite{ATLAS:2023dew}, and $\frac{\Delta\sigma_{VVV}}{\sigma_{VVV}}$ = 24\%~\cite{STDM-2019-09,ATLAS:2024nab}.
...
\begin{table}[htb]
  ...
  \hline
  $Z+\gamma$ & $0.11\pm 0.01$ & $0.14 \pm 0.01$ & $0.18 \pm 0.02$ \\
  $Z$+jets & $0.10 \pm 0.01$ & $0.10 \pm 0.01$ & $0.08 \pm 0.02$ \\
  $t\bar{t}$ & $0.24 \pm 0.04$ & $0.20 \pm 0.04$ & $0.14 \pm 0.03$ \\
  \hline
  \end{tabular}
...
\begin{table}[htb]
  ...
  \hline
  $Z+\gamma$ & $0.16\pm 0.01$ & $0.18 \pm 0.01$ & $0.28 \pm 0.04$ \\
  $Z$+jets & $0.16 \pm 0.01$ & $0.16 \pm 0.01$ & $0.14 \pm 0.02$ \\
  $t\bar{t}$ & $0.27 \pm 0.05$ & $0.27 \pm 0.04$ & $0.17 \pm 0.03$ \\
  \hline
  \end{tabular}
  \caption{Electron fake scale factors calculated from data.}\label{tab:vh_wh_electron_scale_factors_dd}
\end{table}

This method is validated with the same non-closure approach used in the muon fake factors, where the MC-driven scale factors are used to obtain estimates of each background in the SRs, and are then compared to the MC simulation. The resulting plots are seen in Figure~\ref{fig:vh_wh_electron_scale_factors_non_closure_plots} and their corresponding uncertainties in Table~\ref{tab:vh_wh_electron_sf_non_closure_uncertainties}. Closure is good in both SR's for $Z\gamma$ and $t\bar{t}$, while for $Z+$jets the estimated background contribution is much larger than the MC simulation prediction. Since $Z+$jets is such a small part of the total electron fake background and the fake background is a sub-leading background we are not concerned with the large level of non-closure and the corresponding large uncertainty assigned. The final non-closure uncertainty used is taken from whichever SR has the largest non-closure.

\begin{figure}[htp]
  \centering
  \subfloat[$Z\gamma$ closure]{
    \includegraphics[width=0.49\textwidth]{figures/VH/VH_electron_sf_zgamma_nonclosure_combined.pdf}\label{fig:vh_wh_electron_zgamma_non_closure}
  }
  \subfloat[$Z+$jets closure]{
    \includegraphics[width=0.49\textwidth]{figures/VH/VH_electron_sf_zjets_nonclosure_combined.pdf}\label{fig:vh_wh_electron_zjets_non_closure}
  }
  \hfill
  \subfloat[$t\bar{t}$ closure]{
    \includegraphics[width=0.49\textwidth]{figures/VH/VH_electron_sf_ttbar_nonclosure_combined.pdf}\label{fig:vh_wh_electron_ttbar_non_closure}
  }
