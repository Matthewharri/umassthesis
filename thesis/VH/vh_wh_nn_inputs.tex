Each SR in the \wh{} analysis uses a different set of input variables for the ANN training which are optimized per region. The \zdom{} and \zdep{} channels use 23 and 27 variables (listed in Table~\ref{tab:vh_wh_input_variables}), that are selected to maximize the separation between the \wh{} signals and their corresponding backgrounds. The data and MC simulation agreement is checked in dedicated validation regions (VRs), which are defined as sidebands to each signal region, where only a small amount of signal is expected. 
...
\centering
  ...
  \multicolumn{2}{c}{
    \begin{tabular}{l@{: }l}
      $\Delta{\eta}_{\ell_{i}\ell_{j}}(i \neq j)$ & Pseudorapidity separation between leptons in each pair \\
      $\Delta{\varphi_{\ell_{i}\ell_{j}}(i \neq j)}$ & Azimuthal separation between leptons in each pair \\
      $\Delta{R_{\ell_{i}\ell_{j}}(i \neq j)}$ & Angular distance between leptons in each pair \\
      $\Delta{m_{\ell_{i}\ell_{j}}(i \neq j)}$ & Invariant mass of the lepton pairs \\
  ...
  \begin{tabular}{l@{: }l}
    ...
    $\mathrm{N}_{\text{Jets}}$ & Number of jets \\
    $\frac{\metsig}{\met}$ & Ratio of \metsig{} to \met{} \\
  \end{tabular} \\
  \hline
  \end{tabular}
  \caption{Input variables used for training the ANN's in the \wh{} analysis. Common variables are listed under both channels, while region specific variables are listed under their corresponding channel.}\label{tab:vh_wh_input_variables}
\end{table}
...
Due to the limited statistics of the MC samples used for training the \zdep{} ANN, the event selection is loosened to include events with two tight leptons and one anti-ID lepton. Furthermore, the $t\bar{t}$ and $WH$ samples used in the \zdep{} training also contain events that fall into the \zdom{} signal region, as these processes do not depend on SFOS lepton pairs. This choice in the training does not work for $WZ$ events, for example, due to the nature of the $Z$-boson decaying to a SFOS lepton pair.

Although jets are not present in the final state of the \wh{} decay process, where $W \to \ell\nu$ and $H \to WW^{*}\to \ell\nu\ell\nu$, and arise only through initial state radiation (ISR), jet related variables are included in the ANN primarily for $t\bar{t}$ suppression. Jets from $t\bar{t}$ decays generally have larger \pt{} than ISR jets, making the leading jet \pt{} a useful discriminating variable. Additionally, the leading jet \pt{} has discriminating power in the \zdom{} analysis by providing discriminating power against $ZZ$ events. In \wh{} events, a radiated gluon recoils against a single $W$ boson, whereas in $ZZ$ events, it recoils against the $ZZ$ system, leading to harder jets when compared to \wh{}.

Spin correlations in the \hww{} decay cause the \lzero{} and \lone{} leptons to have a small angular separation and small invariant mass, providing further discrimination against diboson and $t\bar{t}$ processes.

Variables related to \met{} offer strong discriminating power, particularly against $t\bar{t}$ events which contain two neutrinos, and the $WZ$ process which involves one to six neutrinos in the \zdom{} channel and seven in the \zdep{} channel. Additionally, the $ZZ$ process has various neutrino combinations, but never the same as the \wh{} process which gives rise to three neutrinos. The difference between the number of neutrinos in the final state for backgrounds compared to the signal makes \met{} related variables especially strong.
...
The input variables used in the \zdom{} channel are shown in Appendix~\ref{app:zdom_input_plots} and those used in the \zdep{} channel are provided in Appendix~\ref{app:zdep_input_plots}. In both cases, the inputs are shown after applying the full event selection.