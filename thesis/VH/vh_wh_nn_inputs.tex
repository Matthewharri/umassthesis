...
\centering
  ...
  \multicolumn{2}{c}{
    \begin{tabular}{l@{: }l}
      $\Delta{\eta}_{\ell_{i}\ell_{j}}(i \neq j)$ & Pseudorapidity separation between lepton pairs \\
...
Variables related to \met{} offer strong discriminating power, particularly against $t\bar{t}$ events which contain two neutrinos, and the $WZ$ process which involves one to seven neutrinos in the \zdom{} channel and seven in the \zdep{} channel. Additionally, the $ZZ$ process has various neutrino combinations, but never the same as the \wh{} processes which contains three neutrinos. The difference between the number of neutrinos in the final state for backgrounds compared to the signal makes \met{} related variables especially strong.
