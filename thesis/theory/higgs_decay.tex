...
To study this decay, we must first produce Higgs bosons which will be discussed in the following Section.

% In the SM, the Higgs boson coupling to fermions is proportional to their mass ($g_{Hff} = \frac{m_{f}}{v}$), and for bosons their mass squared ($g_{HVV} = \frac{2m_{V}^{2}}{v}$). Figure~\ref{fig:coupling_vs_mass} shows reduced coupling constants vs particle mass for the ATLAS experiment. The coupling constants that have been measured so far for the Higgs boson are plotted, confirming that the coupling constants are proportional to the mass of the particles. Couplings to all vector bosons and third generation fermions have been measured, with couplings to second generations fermions underway.

% These coupling constants imply that the Higgs boson prefers to decay into the heaviest kinematically accessible particles. Figure~\ref{fig:Higgs_BR_mass_range} shows the Higgs boson branching ratios as a function of its mass. At higher mass ranges the vector boson decays become the dominant decay modes as the Higgs boson can kinematically decay into two on-shell vector bosons, and the coupling to vector bosons goes as $m_{V}^{2}$. For a Higgs boson of mass $m_{H} = 125$ GeV, the dominant decay modes is $H \rightarrow b\bar{b}$ since this is the heaviest fermion kinematically available for decay. The vector bosons are suppressed due to the requirement of an off-shell vector boson in the decay.