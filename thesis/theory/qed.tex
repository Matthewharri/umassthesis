Quantum electrodynamics (QED) is the description of electromagnetic (EM) interactions between charged particles and photons as a relativistic quantum field theory (QFT). It is an abelian gauge theory with the symmetry group $\mathrm{U}(1)$. As previously mentioned, the $\mathrm{U}(1)$ symmetry group corresponds to phase rotations in the complex plane and represents a global symmetry. The full Lagrangian density for charged Dirac fermions interacting with the EM field is given by
\begin{equation}
...
\end{equation}
the Lagrangian transforms accordingly as
\begin{equation}
...
\end{equation}
where the last term corresponds to the mass of the gauge field. However, this term breaks the local gauge invariance, therefore, the mass of the gauge field must vanish and the final Lagrangian density becomes 
\begin{equation}
  \mathcal{L}_{\mathrm{QED}} = \bar{\psi}(i\gamma^{\mu}D_{\mu} - m)\psi - \frac{1}{4}F^{\mu\nu}F_{\mu\nu.}
