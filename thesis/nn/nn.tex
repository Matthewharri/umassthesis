...
\begin{figure}[htp]
  ...
  \caption{A simple feed-forward neural network. Each network starts with an input layer, can have any number of hidden layers, and ends with an output layer. Each circular node represent a neuron, with an arrow representing a connection from the output of one neuron to the input of another~\cite{ColoredNN}.}\label{fig:ann_image}
\end{figure}
...
\end{equation}
where $y_{i}$ is the truth label, $\hat{y}_{i}$ is the predicted label, and $N$ the number of input vectors. For multi-class classification where an ANN has more than two categories, the categorical cross-entropy is used, defined as:
\begin{equation}
